...
# --- Configuration ---
TARGET_HOST="gladstone"
TARGET_IP="192.168.100.12"
TARGET_USER="a"
SOURCE_DIRS=("/var/lib/gitea/" "/etc/gitea/")
# --- End Configuration ---
...
rsync -avz --delete -e ssh ${TARGET_USER}@${TARGET_IP}:${SOURCE_DIRS[0]} "${CURRENT_BACKUP_DIR}/gitea_data/" >> "${LOG_FILE}" 2>&1
rsync -avz --delete -e ssh ${TARGET_USER}@${TARGET_IP}:${SOURCE_DIRS[1]} "${CURRENT_BACKUP_DIR}/gitea_config/" >> "${LOG_FILE}" 2>&1
